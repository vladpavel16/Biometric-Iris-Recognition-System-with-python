...
def unravel_iris(img, xp, yp, rp, xi, yi, ri, phase_width=300, iris_width=150):
    ...
    if img.ndim > 2:
        img = img[:, :, 0].copy()
    iris = np.zeros((iris_width, phase_width)) #create a 150x300 array with null elements
...
def iris_encode(img, dr=15, dtheta=15, alpha=0.4):
    ...
    norm_iris = (img - img.mean()) / img.std() # image normalization
    patches = view_as_blocks(norm_iris, (dr, dtheta)) #split image to blocks
    code = np.zeros((patches.shape[0] * 3, patches.shape[1] * 2))# large the size of code to add the information from three frequency and from real and imaginary part
...
def preprocess(image):
    ...
    img = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY) #convert image to  grayscale
    return cv2.medianBlur(img, 5) #apply the Median Blur filter with a 5X5 kernel
...
def find_pupil_hough(img):
    ...
    if circles is not None:#verify if a circle is detected
        ...
        return circles[0, 0][0], circles[0, 0][1], circles[0, 0][2] # return x and y coordinates and the radius of the circle
    else:
        return 0,0,0  
...
def show_details(image,image2):
    ...
    plt.title('Segmentation process')
    plt.subplots_adjust(hspace=0.3)
    plt.subplot(212)
    plt.imshow(iris,cmap=plt.cm.gray)
    plt.title('Normalization process')
    f.show()
    ...
    cv2.circle(image2, (x2, y2), r, (255, 0, 0), 3)
    cv2.circle(image2, (x2, y2), 2, (255, 0, 0), 2)
    cv2.circle(image2, (x2_iris, y2_iris), r2_iris, (0, 255, 0), 3)
    cv2.circle(image2, (x2_iris, y2_iris), 2, (0, 255, 0), 2)
